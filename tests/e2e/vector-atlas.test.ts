...
import { dropVectorSearchIndex, ensureVectorSearchIndex } from '../helpers/atlas-search-index.js';
...
describe.skip(`Atlas Vector Search E2E (${gate.reason ?? 'enabled'})`, () => {
  ...
  it('auto-embed → real $vectorSearch → returns scored docs with custom idField intact', async () => {
    ...
    const repo = new Repository<IE2eDoc>(
      Model,
      [methodRegistryPlugin(), vectorPlugin({ fields: [vectorField], embedFn, autoEmbed: true })],
      {},
      { idField: 'docCode' },
    );
