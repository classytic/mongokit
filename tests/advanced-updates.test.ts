...
import type {
  ...
  BulkWriteResult,
} from '../src/plugins/batch-operations.plugin.js';
...
describe('Advanced Update Features', () => {
  ...
  describe('atomicUpdate() — Composite Operators', () => {
    it('should combine $inc and $set in a single atomic operation', async () => {
      ...
      expect(new Date((updated as Record<string, unknown>).lastActiveAt as string).getTime()).toBe(
        now.getTime(),
      );
    });
    ...
    it('should handle $inc + $set + $push (reaction toggle pattern)', async () => {
      ...
      const history = (updated as Record<string, unknown>).history as Array<
    ...
    it('should reject empty operators object', async () => {
      ...
      await expect(postRepo.atomicUpdate(post._id.toString(), {})).rejects.toThrow(
  ...
  describe('bulkWrite() — Heterogeneous Batch Operations', () => {
      ...
      const result = await productRepo.bulkWrite([
        ...
        },
        {
          updateOne: {
            ...
            update: { $inc: { stock: 5 }, $set: { status: 'restocked' } },
          },
        },
    ...
    it('should handle ordered=false for parallel execution', async () => {
      ...
      const p2 = await productRepo.create({
        name: 'P2',
        price: 20,
        stock: 20,
      } as Partial<IProduct>);
    ...
    it('should handle complex real-world scenario: inventory rebalancing', async () => {
      ...
      const low1 = await productRepo.create({
        name: 'Low Stock 1',
        price: 10,
        stock: 2,
        status: 'active',
      } as Partial<IProduct>);
      const low2 = await productRepo.create({
        name: 'Low Stock 2',
        price: 20,
        stock: 1,
        status: 'active',
      } as Partial<IProduct>);
      const empty = await productRepo.create({
        name: 'Empty',
        price: 15,
        stock: 0,
        status: 'active',
  ...
  describe('arrayFilters — Positional Operator Updates', () => {
    ...
    it('should update multiple array elements matching a condition', async () => {
      ...
      const options = (updated.poll as Record<string, unknown>).options as Array<
        Record<string, unknown>
      >;
  ...
  describe('Real-World Scenarios', () => {
    ...
    it('scenario: e-commerce flash sale (bulkWrite)', async () => {
      ...
      const p3 = await productRepo.create({
        name: 'Doohickey',
        price: 25,
        stock: 200,
        status: 'active',
