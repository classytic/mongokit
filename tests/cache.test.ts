...
describe("Cache Plugin", () => {
  ...
  describe("Cache Hit/Miss", () => {
    ...
    it("should skip cache when skipCache option is true", async () => {
      ...
      const skipId = user._id.toString();
      ...
      await userRepo.getById(skipId);
      expect(setCalls).toBe(1);

      // Skip cache call -- should still return data but bypass cache read
      const result = await userRepo.getById(skipId, { skipCache: true });
    ...
    it("should cache getAll results", async () => {
      ...
      // get() called 3 times: version read (before), key lookup (before), version read (after)
      const result1 = await userRepo.getAll({ filters: { status: "active" } });
      expect(result1.docs.length).toBe(2);
      expect(getCalls).toBe(3); // version (before) + key miss + version (after)
      expect(setCalls).toBe(1); // store list result

      // Second call - cache hit
      // get() called 2 times: version read (before) + key lookup (HIT — no after:getAll write path)
      getCalls = 0;
      setCalls = 0;
      const result2 = await userRepo.getAll({ filters: { status: "active" } });
      expect(result2.docs.length).toBe(2);
      expect(getCalls).toBe(2); // version (before) + key hit
      expect(setCalls).toBe(0); // No new set (cache hit)
    });
  });
